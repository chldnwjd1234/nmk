//탭 클릭 이벤트와 select 클릭했을 때의 상황 + svg 표시 관련 코드(svg는 제가 해본겁니다..)
// 층 탭 클릭 이벤트
...
document.addEventListener("DOMContentLoaded", () => {
  const artImgs = document.querySelectorAll(".highlighted_artifact .art_1f .img_box img");
  const svgItems = document.querySelectorAll(".map_right .f1 .product_svg li");
  ...
  artImgs.forEach((img, index) => {
    img.addEventListener("click", () => {
      ...
      // 클릭한 index에 해당하는 SVG 표시
      if (svgItems[index]) {
        svgItems[index].style.display = "block";
      }
    });
  });
});
